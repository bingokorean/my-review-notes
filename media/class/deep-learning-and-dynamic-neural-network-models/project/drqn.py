...
class Qnetwork():
	...
	def __init__(self, h_size, rnn_cell, myScope):
		...
		self.W1 = self.weight_variable([OBSERVATION_DIM, h_size])
		self.b1 = self.bias_variable([h_size])
		self.W2 = self.weight_variable([h_size, h_size])
		self.b2 = self.bias_variable([h_size])
		self.W3 = self.weight_variable([h_size, h_size])
		self.b3 = self.bias_variable([h_size])

		self.scalarInput =  tf.placeholder(tf.float32, [None, OBSERVATION_DIM]) # input: state
		self.h1 = tf.nn.relu(tf.matmul(self.scalarInput, self.W1)  + self.b1)
		self.h2 = tf.nn.relu(tf.matmul(self.h1, self.W2) + self.b2)
		self.h3 = tf.nn.relu(tf.matmul(self.h2, self.W3) + self.b3)
		self.batch_size = tf.placeholder(dtype=tf.int32)
		self.trainLength = tf.placeholder(dtype=tf.int32)
		self.convFlat = tf.reshape(slim.flatten(self.h3), [self.batch_size, self.trainLength, h_size])
		...
		self.AW = tf.Variable(tf.random_normal([h_size//2, ACTION_DIM]))
...
with tf.Session() as sess:
	...
	#print(s)
        ...
        while j < max_epLength: # maximum time step = 1600
            j+=1


            #Choose an action by greedily (with e chance of random action) from the Q-network
            if np.random.rand(1) < 0.01 and total_steps < pre_train_steps:
                state1 = sess.run(mainQN.rnn_state,\
                    feed_dict={mainQN.scalarInput:[s],mainQN.trainLength:1,mainQN.state_in:state,mainQN.batch_size:1}) # FNN
		...
		a = np.zeros(ACTION_DIM)
		#print(action_index)
                a[action_index] = 1 # always [1, 0] until training hidden layer
	    
	    
	    #print('--------------> ', a)
            s1,r,d,_ = env.step(np.argmax(a))
	    print(np.argmax(a))
            #s1 = processState(s1P)
            total_steps += 1
            if d == True:
		r = -1000
            ...
            episodeBuffer.append(np.reshape(np.array([s,np.argmax(a),r,s1,d]),[1,5]))
		    ...
		    # trainBatch => [state, action, reward, next_state, done]
		    #print(trainBatch)
		    #print('\n')
	 	    ...
	 	    #print("---> before!!!")
		    #print(np.vstack(trainBatch[:,0])) # current state	
		    #print('\n')
		    #print(targetQ) # [x] * batch_size
		    #print('\n')
		    #print(trainBatch[:,1]) # [action dim] * batch_size
		    #print(trace_length)
		    #print(state_train)
		    #print(batch_size)
                    ...
                    sess.run(mainQN.updateModel, \
                        ...
                        mainQN.actions:list(trainBatch[:,1]), mainQN.trainLength:trace_length,\
                        mainQN.state_in:list(state_train), mainQN.batch_size:batch_size})
	...
	print('------------episode =', i, '---- total reward =', rAll)
