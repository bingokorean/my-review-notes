...
#print(env.spec.timestep_limit) = 200
...
class DQN:
	...
	def _build_network(self, h_size=128, l_rate=1e-1):
...
def replay_train(mainDQN, targetDQN, train_batch):
	...
	for state, action, reward, next_state, done in train_batch:
		...
		if done:
			...
			if SEPARATE_TRICK == True:
				# get target from target DQN (Q')
				Q[0, action] = reward + dis * np.max(targetDQN.predict(next_state))
			else: # do not use separate trick
		...
		for episode in range(max_episodes):
			...
			while not done:
				if np.random.rand(1) < e:
					...
					prediction = mainDQN.predict(state)
					action = np.argmax(prediction)
					#print('before argmax ==>', prediction)
					#print('aftet argmax ==>', action)
				...
				#print('reward ==>', reward)
				if done and step_count < 195: # Penalty
				...
				state = next_state
				step_count += 1
				#if step_count > 10000: # Good enough. Let's move on -> WRONG! 200 is maximum
				#	break
			
			

			if fail_token == 1:
				total_score = reward_sum + bad_reward*(-1)
			else:
				total_score = reward_sum
			print("Episode: {}, total reward: {},  steps: {}".format(episode, total_score, step_count))

			# Game over
			pandas_reward.append(total_score)
			...
			#	for _ in range(50):
			#		minibatch = random.sample(replay_buffer, 256)
