...
path = get_file( # get_file함수를 사용함으로써, 다운로드는 따로 할 필요가 없음.
...
def get_stories(f):
    ...
    for line in f:
		...
		nid, line = line.split(' ', 1) # each story는 1번부터 시작함.
...
def should_flatten(el): 
    return not isinstance(el, (str, bytes)) # list of words를 flatten해줌. 
...
# embedded_question.shape     = (N, 1, embedding_dim) # shape를 맞추기 위해 가운데 1을 추가해서 dim을 늘림.
...
# story weight를 출력하기 위해 따로 debug_model을 생성.
...
def hop(query, story): # query: question or answer
    ...
    story_embedding2 = embed_and_sum(input_story_) # for 2 supporting fact
