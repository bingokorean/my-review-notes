...
##### build the model #####
# attention 메커니즘을 구현하는데 있어서 encoder는 동일하고, decoder만 변형한다.
...
decoder_inputs_placeholder = Input(shape=(max_len_target,)) # teacher forcing input
...
######### Attention #########
# Attention layers need to be global (전역 변수) because
...
# 원래 LSTM은 내부적으로 위오 같은 for문을 진행하지만, 여기서 우리는 Context를 계산하기 위해서 manual하게 for문을 구성함.
...
def stack_and_transpose(x): # 다시 원래의 shape로 만들기 위해.
