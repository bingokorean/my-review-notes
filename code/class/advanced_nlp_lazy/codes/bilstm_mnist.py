...
input_ = Input(shape=(D, D)) # 여기선 T가 D이다.
...
# custom layer
permutor = Lambda(lambda t: K.permute_dimensions(t, pattern=(0, 2, 1))) # 이미지를 90도 회전하기 위해
...
# compile
model.compile(
	# 일반적인 categorical_crossentropy는 one-hot encoded target을 필요로 함. (큰 space를 차지함)
    loss='sparse_categorical_crossentropy', # target이 one-hot encoding되지 않아도 됨. 
    optimizer='adam',
    metrics=['accuracy']
)
